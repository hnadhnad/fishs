...
// Spawns bombs at two vertical columns (left/right) then moves them into a circle around center
    private void SpawnAndArrangeBombs(Boss boss, Vector3 center)
    {
        ...
        // === tạo target positions nguyên vòng tròn ===
        ...
        for (int i = 0; i < half; i++)
        {
            ...
            // ✅ target đúng nửa vòng trái (0..half-1)
            Vector3 target = circleTargets[i];
            Vector3 finalTarget = new Vector3(target.x, target.y, target.z);
        ...
        for (int i = 0; i < half; i++)
        {
            ...
            // ✅ target đúng nửa vòng phải (half..total-1)
            Vector3 target = circleTargets[i + half];
            Vector3 finalTarget = new Vector3(target.x, target.y, target.z);

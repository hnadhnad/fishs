...
public class BossPhase3State : IBossState
{
    ...
    public void SpawnMeatOnBombHit(Boss boss)
    {
        ...
        // Trường hợp vòng spawn nằm hoàn toàn trong vòng bomb -> spawn đủ 360°
        if (d + r <= R + EPS)
        {
        ...
        // Trường hợp không giao nhau (vòng nhỏ hoàn toàn ngoài vòng lớn) -> không spawn
        if (d >= R + r - EPS)
        {
            return;
        }

        // Tính giao điểm 2 đường tròn
        ...
        // Nếu chỉ tiếp tuyến -> spawn tại 1 điểm
        if (arcSpan <= 1e-3f)
        {
        ...
        // Spawn đều trên cung [startAngle, startAngle + arcSpan]
        for (int i = 0; i < count; i++)
        {
            float ang;
            if (count == 1)
                ang = startAngle + arcSpan * 0.5f;
            else
                ang = startAngle + arcSpan * i / (count - 1);

            Vector3 dir = new Vector3(Mathf.Cos(ang), Mathf.Sin(ang), 0f);
            Vector3 pos = bossPos + dir * r;

            // an toàn: nếu lệch ra ngoài, clamp lại

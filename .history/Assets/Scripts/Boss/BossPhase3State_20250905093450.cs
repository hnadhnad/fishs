...
// Spawns bombs at two vertical columns (left/right) then moves them into a circle around center
    private void SpawnAndArrangeBombs(Boss boss, Vector3 center)
    {
        ...
        for (int i = 0; i < total; i++)
        {
            ...
            float y = center.y + Mathf.Lerp(
                columnSpacingY * 0.5f,
                -columnSpacingY * 0.5f,
                (float)(fromLeft ? i : (i - half)) / Mathf.Max(1, half - 1)
            );
            ...
            // ✅ chọn targetIndex đúng nửa vòng
            int targetIndex;
            if (fromLeft)
            {
                // Trái đi vào nửa vòng bên trái (180°–360°) → tức indices [half..total-1]
                targetIndex = half + i;
            }
            else
            {
                // Phải đi vào nửa vòng bên phải (0°–180°) → tức indices [0..half-1]
                targetIndex = i - half;
            }

            Vector3 finalTarget = circleTargets[targetIndex];

...
// Spawns bombs at two vertical columns (left/right) then moves them into a circle around center
    private void SpawnAndArrangeBombs(Boss boss, Vector3 center)
    {
        ...
        Vector3[] leftTargets = new Vector3[half];
        Vector3[] rightTargets = new Vector3[half];

        // Trái: từ 90° + stepLeft → 270°
        float stepLeft = 180f / half;
        for (int i = 0; i < half; i++)
        {
            float ang = (90f + stepLeft * (i + 1)) * Mathf.Deg2Rad; // bắt đầu sau 90°
            Vector3 dir = new Vector3(Mathf.Cos(ang), Mathf.Sin(ang), 0f);
            leftTargets[i] = center + dir * boss.phase3CircleRadius;
        }

        // Phải: từ 90° - stepRight → -90°
        float stepRight = 180f / half;
        for (int i = 0; i < half; i++)
        {
            float ang = (90f - stepRight * (i + 1)) * Mathf.Deg2Rad; // kết thúc trước 90°
            Vector3 dir = new Vector3(Mathf.Cos(ang), Mathf.Sin(ang), 0f);

...
public class BossPhase1State : IBossState
{
    ...
    private IEnumerator DashToPlayer(Boss boss)
    {
        ...
        // --- 1. Tiến gần player đến khi đủ điều kiện dash ---
        float dashRange = boss.phase1DashDistance * 0.8f; // ngưỡng để dash
        while (Vector3.Distance(boss.transform.position, player.transform.position) > dashRange)
        {
            Vector3 oldPos = boss.transform.position;
            boss.transform.position = Vector3.MoveTowards(
                boss.transform.position,
                player.transform.position,
                boss.moveSpeed * Time.deltaTime
            );
            boss.UpdateVisualFromDelta(boss.transform.position - oldPos);
            yield return null;
        }

        // --- 2. Dash thật sự (dùng Boss.DashRoutine) ---
        yield return boss.StartCoroutine(
            boss.DashRoutine(player.transform.position, boss.phase1DashDistance, boss.phase1DashDuration)
        );

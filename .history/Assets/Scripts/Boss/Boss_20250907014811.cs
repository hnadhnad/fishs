...
public class Boss : MonoBehaviour
{
    ...
    public void UpdateVisualFromDelta(Vector3 delta)
    {
        ...
        // nếu gần như đứng yên → trở về trạng thái mặc định dần dần
        if (moveDir.magnitude < 0.01f)
        {
            ...
            transform.localScale = new Vector3(transform.localScale.x, // giữ hướng cũ
                                            currentSize,
                                            currentSize);
            return;
        }

        // 🔥 Flip X theo hướng di chuyển, nhưng có deadzone tránh rung
        if (Mathf.Abs(moveDir.x) > 0.05f) // chỉnh ngưỡng này trong editor nếu muốn
        {

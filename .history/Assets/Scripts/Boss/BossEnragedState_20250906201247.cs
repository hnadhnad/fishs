...
public class BossEnragedState : IBossState
{
    ...
    private IEnumerator EnragedRoutine(Boss boss)
    {
        ...
        if (shieldSaved)
        {
            ...
            // đặt player vào rìa trái map trong bụng boss
            Vector3 leftEdge = new Vector3(map.bottomLeft.x, 0f, 0f);
            playerGO.transform.position = leftEdge;
        }
        else
        {
            ...
            {
                fish.Die();
                yield break;   // ✅ STOP coroutine ngay khi player bị kill
            }
        }

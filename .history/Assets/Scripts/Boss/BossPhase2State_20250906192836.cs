...
public class BossPhase2State : IBossState
{
    ...
    private IEnumerator PhaseRoutine(Boss boss)
    {
        ...
        var centerPos = Vector3.zero; // toạ độ giữa map
        ...
        while (boss != null && boss.currentHealth > 0f)
        {
            ...
            // 🔒 khóa phase transition khi đang chạy routine chính
            boss.allowPhaseTransition = false;

            // Boss giữ vị trí giữa map (nếu lệch sau khi ăn thịt)
            ...
            // 🔁 Pattern loop
            yield return BombThenShoot(boss, playerT, 3);
            yield return BombThenShoot(boss, playerT, 2);
            yield return BombThenShoot(boss, playerT, 1);
            yield return BombThenShoot(boss, playerT, 0);

            // ✅ Sau pattern → boss đi ăn thịt (nếu có)
            yield return EatAllMeat(boss);

            // 🔓 mở lại phase transition sau khi ăn xong

...
public class BossPhase1State : IBossState
{
    ...
    private IEnumerator RetreatAndLure(Boss boss)
    {
        ...
        if (boss.phase1LurePrefab != null)
        {
            ...
            // set hướng di chuyển cho LureFish
            LureFish lf = lure.GetComponent<LureFish>();
            if (lf != null)
            {
                lf.moveDirection = goLeft ? Vector2.right : Vector2.left; // luôn bơi vào trong map
                lf.speed = boss.phase1LureSpeed;
            }
        }

        // Boss chase lure cho tới khi lure biến mất hoặc ăn được
        while (lure != null)
        {
            boss.transform.position = Vector3.MoveTowards(
                boss.transform.position,

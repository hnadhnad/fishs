...
// Spawns bombs at two vertical columns (left/right) then moves them into a circle around center
    private void SpawnAndArrangeBombs(Boss boss, Vector3 center)
    {
        ...
        // target positions theo vòng tròn
        ...
        for (int i = 0; i < total; i++)
        {
            ...
            // chọn vị trí trên vòng tròn
            int targetIndex = i;
            Vector3 circleTarget = circleTargets[targetIndex];

            // ✅ chỉ lấy X từ vòng tròn, giữ nguyên Y ban đầu
            Vector3 finalTarget = new Vector3(circleTarget.x, spawnPos.y, circleTarget.z);

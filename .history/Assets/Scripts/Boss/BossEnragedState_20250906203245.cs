...
public class BossEnragedState : IBossState
{
    ...
    private IEnumerator EnragedRoutine(Boss boss)
    {
        ...
        // Nếu fish đã được cứu trước đó (ví dụ Fish.Die() đã bị shield block),
        // coi là được cứu — tránh gọi Die() lại lần nữa.
        if (fish != null && fish.wasSavedByShield)
        {
            shieldSaved = true;
            // reset flag (nếu muốn giữ trạng thái chỉ 1 lần)
            fish.wasSavedByShield = false;
            Debug.Log("[BossEnragedState] Detected fish.wasSavedByShield -> treat as saved");
        }
        ...
        if (shieldSaved)
        {
            ...
            Vector3 spawnPos = new Vector3(map.bottomLeft.x + 1f, (map.bottomLeft.y + map.topRight.y) / 2f, playerGO.transform.position.z);
        ...
        else
        {
            ...
            // Final attempt to kill. Fish.Die() sẽ lại kiểm tra shield (an toàn).
            if (fish != null)
            {
                fish.Die();
            }

...
public class Boss : MonoBehaviour
{
    ...
    public void UpdateVisualFromDelta(Vector3 delta)
    {
        ...
        // Nếu gần như đứng yên => đưa về rotation mặc định
        if (moveDir.magnitude < 0.01f)
        {
            ...
            transform.localScale = new Vector3(baseScaleX, baseScale.y, baseScale.z);
            return;
        }

        // ✅ Flip X theo hướng di chuyển, nhưng giữ nguyên tỷ lệ gốc
        if (Mathf.Abs(moveDir.x) > 0.01f)
        {
            float signX = Mathf.Sign(moveDir.x);
            transform.localScale = new Vector3(signX * baseScaleX, baseScale.y, baseScale.z);
        }

...
public class BossPhase3State : IBossState
{
    ...
    public void SpawnMeatOnBombHit(Boss boss)
    {
        ...
        Debug.Log($"[SpawnMeat] bossPos={bossPos}, R={R}, r={r}, d={d}, count={count}");

        // fully inside -> full circle
        if (d + r <= R + EPS)
        {
            float stepFull = 2f * Mathf.PI / count;
            for (int i = 0; i < count; i++)
            {
                float ang = stepFull * i;
                Vector3 dir = new Vector3(Mathf.Cos(ang), Mathf.Sin(ang), 0f);
        ...
        Debug.Log($"[SpawnMeat] intersection: a1={a1}, a2={a2}, start={startAngle}, arcSpan={arcSpan}");
        ...
        // DISTRIBUTE: use (i+0.5)/count to avoid endpoints; if you prefer exact endpoints use i/(count-1)
        for (int i = 0; i < count; i++)
        {
            float ang;
            if (count == 1) ang = startAngle + arcSpan * 0.5f;
            else ang = startAngle + arcSpan * (i + 0.5f) / count;

            Vector3 dir = new Vector3(Mathf.Cos(ang), Mathf.Sin(ang), 0f);
            Vector3 pos = bossPos + dir * r;

            // clamp if numeric error

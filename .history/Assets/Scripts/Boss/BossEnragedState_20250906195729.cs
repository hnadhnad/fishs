...
public class BossEnragedState : IBossState
{
    ...
    private IEnumerator EnragedRoutine(Boss boss)
    {
        ...
        if (shieldSaved)
        {
            ...
            // change background sprite via MapManager method if available
            if (boss.enragedBackground != null)
            {
                map.ChangeBackground(boss.enragedBackground);
            }

            // optional: place player near some inside-boss spawn point (here we place at boss pos offset)
            playerGO.transform.position = boss.transform.position + new Vector3(0.5f, 0f, 0f);
        }
        else
        {

...
public class BossPhase1State : IBossState
{
    ...
    private IEnumerator RetreatAndLure(Boss boss)
    {
        ...
        // ✅ ra ngoài map (thêm exitDistance) để player không thấy
        ...
        while (Vector3.Distance(boss.transform.position, exitPos) > 0.1f)
        {
            boss.transform.position = Vector3.MoveTowards(
                ...
                exitPos,
                boss.phase1ExitSpeed * Time.deltaTime
            );
            yield return null;
        }
        ...
        // Spawn lure ngay trước mặt boss
        ...
        // ✅ boss đi vào lại rìa map (không đứng ngoài luôn)
        ...
        while (Vector3.Distance(boss.transform.position, reentryPos) > 0.1f)
        {
            boss.transform.position = Vector3.MoveTowards(
                boss.transform.position,
                reentryPos,
                boss.phase1ReturnSpeed * Time.deltaTime
            );
            yield return null;
        }
    }

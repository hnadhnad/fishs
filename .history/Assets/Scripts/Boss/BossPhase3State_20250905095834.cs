...
// Spawns bombs at two vertical columns (left/right) then moves them into a circle around center
    private void SpawnAndArrangeBombs(Boss boss, Vector3 center)
    {
        ...
        int halfLeft = total / 2;
        int halfRight = total - halfLeft;

        // === tạo target positions ===
        Vector3[] leftTargets = new Vector3[halfLeft];
        Vector3[] rightTargets = new Vector3[halfRight];

        // Trái: 90° → 270°
        float stepLeft = 180f / (halfLeft - 1);
        for (int i = 0; i < halfLeft; i++)
        {
            float ang = (90f + stepLeft * i) * Mathf.Deg2Rad;
            Vector3 dir = new Vector3(Mathf.Cos(ang), Mathf.Sin(ang), 0f);
            leftTargets[i] = center + dir * boss.phase3CircleRadius;
        }

        // Phải: 90° → -90°
        float stepRight = 180f / (halfRight - 1);
        for (int i = 0; i < halfRight; i++)
        {
            float ang = (90f - stepRight * i) * Mathf.Deg2Rad;
            Vector3 dir = new Vector3(Mathf.Cos(ang), Mathf.Sin(ang), 0f);
            rightTargets[i] = center + dir * boss.phase3CircleRadius;
        }
        ...
        for (int i = 0; i < halfLeft; i++)
        {
            float y = center.y + Mathf.Lerp(columnSpacingY * 0.5f, -columnSpacingY * 0.5f, (float)i / (halfLeft - 1));
        ...
        for (int i = 0; i < halfRight; i++)
        {
            float y = center.y + Mathf.Lerp(columnSpacingY * 0.5f, -columnSpacingY * 0.5f, (float)i / (halfRight - 1));

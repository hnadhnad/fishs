...
public class BossPhase1State : IBossState
{
    ...
    private IEnumerator RetreatAndLure(Boss boss)
    {
        ...
        // Spawn lure trước mặt boss, đi vào map
        ...
        if (boss.phase1LurePrefab != null)
        {
            ...
            if (lf != null)
            {
                lf.moveDirection = forward;       // bơi theo hướng vào map
                lf.speed = boss.phase1LureSpeed;  // lấy speed từ boss
            }
        }

        // Boss chase lure cho đến khi lure bị ăn hoặc đi khỏi map
        while (lure != null)
        {
            boss.transform.position = Vector3.MoveTowards(
                boss.transform.position,

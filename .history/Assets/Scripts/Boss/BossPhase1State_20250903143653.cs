...
public class BossPhase1State : IBossState
{
    ...
    private IEnumerator RetreatAndLure(Boss boss)
    {
        ...
        if (boss.phase1LurePrefab != null)
        {
            ...
            // setup lure di chuyển ngang map
            Rigidbody2D lureRb = lure.GetComponent<Rigidbody2D>();
            if (lureRb != null)
            {
                lureRb.isKinematic = true;
                lureRb.gravityScale = 0f;

                Vector3 targetPos = goLeft
                    ? new Vector3(map.topRight.x + 2f, pos.y, pos.z)
                    : new Vector3(map.bottomLeft.x - 2f, pos.y, pos.z);
        ...
        // Boss chase lure tới khi lure biến mất hoặc bị ăn
        while (lure != null)
        {
            boss.transform.position = Vector3.MoveTowards(
                boss.transform.position,
        ...
        // lure tự hủy khi ra khỏi map mà boss chưa ăn → không heal
        if (!ateLure && lure != null)
        {
            GameObject.Destroy(lure);
        }

        // boss vào lại trong map

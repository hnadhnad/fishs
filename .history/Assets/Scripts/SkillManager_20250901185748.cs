...
public class SkillManager : MonoBehaviour
{
    ...
    public void UseSkill(int skillId)
    {
        switch (skillId)
        {
            ...
            case 2:
                FishMovement fishMove = FindObjectOfType<FishMovement>();
                if (fishMove != null)
                {
                    fishMove.enableDash = true; // bật dash
                    Debug.Log("Skill: Bật Dash!");
                }
                break;

            case 3:
                PlayerHunger.Instance.hungerDecayRate *= 0.5f; // giảm còn một nửa
                break;

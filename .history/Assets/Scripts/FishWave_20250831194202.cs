...
public class FishWave : MonoBehaviour
{
    ...
    void Update()
    {
        if (!fleeing && player != null && playerFish != null && selfFish != null)
        {
            float distance = Vector2.Distance(transform.position, player.position);

            // So sánh có thêm margin để tránh trường hợp gần bằng
            if (distance <= fleeRadius && playerFish.size > selfFish.size * 1.05f)
            {
                fleeing = true;
            }
        }
        ...
        if (fleeing)
        {
            // luôn tính fleeDir ra xa player
            fleeDir = (transform.position - player.position).normalized;

            // check chắc chắn là nó chạy xa chứ không dí nhầm
            Vector2 toPlayer = (player.position - transform.position).normalized;
            if (Vector2.Dot(fleeDir, toPlayer) > 0)
            {
                fleeDir = -fleeDir; // đảo ngược nếu hướng sai
            }

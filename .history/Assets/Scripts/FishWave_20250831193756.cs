...
public class FishWave : MonoBehaviour
{
    ...
    void Update()
    {
        if (!isFleeing && player != null && playerFish != null && selfFish != null)
        {
            float distance = Vector2.Distance(transform.position, player.position);

            // Nếu player to hơn trong bán kính -> bắt đầu bỏ chạy
            if (distance <= fleeRadius && playerFish.size > selfFish.size)
            {
                isFleeing = true;
                fleeDir = (transform.position - player.position).normalized; // chỉ tính 1 lần
            }
        }

        if (isFleeing)
        {
            // chạy thẳng ra xa, không đổi hướng theo player nữa
            transform.position += fleeDir * speed * 1.5f * Time.deltaTime;
        }
        else
        {
            // Nếu không bỏ chạy thì bơi sóng, nhưng chỉ dựa vào direction gốc
            SwimWave();
        }
    ...
    void SwimWave()
    {
        ...
        // chỉ đi thẳng theo "direction" ban đầu, không liên quan tới player
